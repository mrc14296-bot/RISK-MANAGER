...
def get_tp1_and_sl_orders(user_id):
    try:
        ...
        for o in all_conditional:
            ...
            is_tp2 = (label == 'TP2') or (order_type == 'LIMIT' and str(o.get('reduceOnly', '')).lower() == 'true')
            is_sl = (label == 'SL') or (('STOP' in order_type) and ('TRAILING' not in order_type))

            if not is_tp1 and not is_sl and not is_tp2:
                continue
